...
engine = pyttsx3.init()
print("\n\t\t\t\t\t\t*************************")
print("\t\t\t\t\t\tWELCOME TO MINI ASSISTANT")
print("\t\t\t\t\t\t*************************")
print("Hey!!\n\nHow Do You Like To listen Me in Male Voice or in female voice\n\nFor Male Voice press 0 or For Female voice"
      " press 1:-", end="")
p = int(input())
...
print(
    "\n'NOTE' - If In Output You Get An Error Name Related To Input Or Output Command.This Means Your Path Is Not Set "
    "Which U Want To Open.")
if p == 1:
    engine.setProperty('rate', 180)
    voices = engine.getProperty("voices")
    engine.setProperty("voice", "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\TTS_MS_EN-US_ZIRA_11.0")
    engine.say("hello I am your mini assistant ")
...
while True:
    ...
    if "exit" in Input or "quit" in Input or "stop" in Input:
        ...
        if ("wmplayer" in Input) or ("player" in Input) or ("media" in Input) or ('wmplayer' in Input) or (
            ...
            engine.runAndWait()
        elif "vlc" not in Input or "player" not in Input or "media" not in Input or 'wmplayer' not in Input \
                or 'window media player' not in Input:
